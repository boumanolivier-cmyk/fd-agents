...
class FallbackChartAgent:
    ...
    def _is_style_change_request(self, message: str) -> bool:
        ...
        has_color_keyword = any(
            keyword in message_lower for keyword in self.bnr_keywords + self.fd_keywords
        )
    ...
    async def analyze(
        self, user_message: str, conversation_history: Optional[List[Dict[str, Any]]] = None
    ) -> ChartData:
        """
        ...
        try:
            ...
            if not data:
                ...
                if conversation_history and any(
                    ...
                    for keyword in ["previous", "same", "earlier", "last", "that"]
                ):
                    previous_data = self._get_previous_chart_data(conversation_history)
                    if previous_data:
                        data = previous_data
            ...
            return ChartData(
                is_valid=False, reason=f"I encountered an error processing your request: {str(e)}"
            )
