    ...
    def _determine_chart_type(
        ...
        if len(x_labels) >= 3:
            try:
                ...
                nums = [int(re.search(r'\d{4}', label).group()) for label in x_labels[:3] if re.search(r'\d{4}', label)]
        ...
        if len(x_labels) >= 10:
            ...
            if all(str(i) in label or f"{i:02d}" in label for i, label in enumerate(x_labels[:5], 1)):
                return "line"
